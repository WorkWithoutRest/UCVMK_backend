...
public class Course_clientDaoImpl implements Course_clientDao{
    ...
    public List<Integer> findClient(int id) {
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("SELECT * from course_client WHERE cource_id=?");
    ...
    public List<Integer> findCourse(int id) {
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("SELECT * from course_client WHERE client_id=?");
            preparedStatement.setInt(1, id);

            ResultSet resultSet = preparedStatement.executeQuery();
            List<Integer> courses=new ArrayList<Integer>();
            while (resultSet.next()) {
                courses.add(resultSet.getInt(2));
            }
            return courses;
        }
        catch (SQLException e)
